...
class AuthController
{
    ...
    public function login()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($data_mahasiswa['password'] != $password) {
    ...
    private function generateNIM($id_pendaftaran){
        ...
        if ($kode_jurusan != 0){
            ...
            $nim  = $kode_jurusan + $tahunmasuk + str_pad($id_pendaftaran);
                    //      - Return nim
            return $nim;
        } else {
    ...
    public function register_step_1()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            //    - Set session id_pendaftaran dengan $id_pendaftaran
                $_SESSION["id_pendaftaran"] = $data_pendaftaran["data_pendaftaran"];
            }
    ...
    public function register_step_2() 
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            //    - Buat query untuk cek apakah NIM sudah ada di database dan simpan di variabel $query_check_nim
                $query_check_nim = "SELECT SELECT COUNT($nim)FROM table_name WHERE unique_key = value"; 
            ...
            //      - Buat query INSERT untuk menyimpan data mahasiswa (nim, id_pendaftaran, password, nama, jurusan)
                    $insert = "INSERT INTO mahasiswa ('nim', 'id_pendaftaran', 'password', 'nama', 'jurusan') VALUES ($nim, $id_pendaftaran, $password, $nama, $jurusan)";
            ...
            //      - Jika eksekusi query berhasil:
                    if (mysqli_num_rows($result_insert) == 1){
